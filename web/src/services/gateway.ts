...
export type FormValues = { name: string; domain: string; serviceName: string; rpcTimeout: string; url?: string };
...
export function saveApp(values: FormValues, editing?: Application) {
 ...
 ? api<{ changed: boolean }>(`/${encodeURIComponent(editing.name)}/update`, 'POST', {
   domain: values.domain.trim(), serviceName: values.serviceName, rpcTimeout: values.rpcTimeout, ...(source ? { idl: source } : {}),
 })
 : api<Application>('', 'POST', { name: values.name, domain: values.domain.trim(), serviceName: values.serviceName,
